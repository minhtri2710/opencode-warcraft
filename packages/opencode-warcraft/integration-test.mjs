...
import { WorktreeService } from 'warcraft-core';

class IntegrationTester {
  constructor() {
    this.worktreeService = new WorktreeService({
      baseDir: process.cwd(),
      warcraftDir: '.beads/artifacts'
    });
  ...
  async setup() {
    ...
    const featureDir = path.join('.beads/artifacts', 'features', this.featureName);
  ...
  async testWorktreeCreation() {
    ...
    try {
      ...
      const worktreePath = path.join('.beads/artifacts', '.worktrees', this.featureName, this.stepFolder);
  ...
  async testApplyToMain(diffResult, baseCommit) {
    ...
    try {
    // Use v2 layout: tasks/ instead of execution/
    const stepDir = path.join('.beads/artifacts', 'features', this.featureName, 'tasks', this.stepFolder);
    await fs.writeFile(
  ...
  async run() {
    console.log('🧪 Warcraft Integration Test Suite');
