      ...
      if ((planStatus === 'instant' || planStatus === 'none' || !planStatus) && pendingCount > 0) {
        if (workflowRecommendation === 'standard' || pendingCount > 2) {
          return pendingCount > 2
            ...
            : 'This task looks broad enough for the standard reviewed path. Use warcraft_task_expand to promote the pending manual task into a reviewed plan, then approve it before dispatching work.';
        }
        if ((workflowPath === 'instant' || workflowRecommendation === 'instant') && pendingCount > 1) {
          return 'This instant workflow now has multiple pending tasks and has likely outgrown the tiny-task path. Use warcraft_task_expand to promote the pending manual tasks into a lightweight plan, then approve it before dispatching more work.';
        }
        if (workflowRecommendation === 'lightweight') {
          return 'This task no longer looks tiny enough for direct execution. Use warcraft_task_expand to promote the pending manual task into a lightweight plan, then approve it before dispatching work.';
        }
      }
