...
import { normalizePath } from 'warcraft-core';
 ...
 *
 * The workspace root is identified as the directory that contains either
 * `.beads/artifacts` (beadsMode on) or `docs` (beadsMode off).
 * Returns null if neither directory is found.
 */
export function findWorkspaceRoot(startDir: string): string | null {
  try {
    let current = path.resolve(startDir);
    while (true) {
      const beadsPath = path.join(current, '.beads/artifacts');
      const docsPath = path.join(current, 'docs');
      if (
        (fs.existsSync(beadsPath) && fs.statSync(beadsPath).isDirectory()) ||
        (fs.existsSync(docsPath) && fs.statSync(docsPath).isDirectory())
      ) {
        return current;
      }
