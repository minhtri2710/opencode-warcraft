---
name: context7-cli
description: Use the ctx7 CLI to fetch library documentation and manage AI coding skills. Activate when the user mentions "ctx7" or "context7", needs current docs for any library, or wants to install/search/generate skills.
---
...
- **[Setup](references/setup.md)** — Configure Context7 CLI for your AI coding agent.
...
# Setup
ctx7 setup --cli                      # Configure Context7 CLI + Skills mode
...
Most commands work without login. Exceptions: `skills generate` always requires it; `ctx7 setup` requires it unless `--api-key` is passed. Login also unlocks higher rate limits on docs commands.
