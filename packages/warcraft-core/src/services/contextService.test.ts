...
describe('ContextService', () => {
  ...
  describe('read/compile/delete', () => {
    it('writes, reads, compiles, and deletes context files', () => {
      const featureName = 'read-compile-delete';
      setupFeature(featureName);

      service.write(featureName, 'research', 'Context A');
      service.write(featureName, 'decisions.md', 'Context B');

      expect(service.read(featureName, 'research')).toBe('Context A');
      expect(service.read(featureName, 'decisions')).toBe('Context B');

      const compiled = service.compile(featureName);
      expect(compiled).toContain('## research');
      expect(compiled).toContain('## decisions');
