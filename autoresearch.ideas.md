...
- `warcraft_worktree_discard` doesn't validate task existence before attempting worktree removal and status transition. The error surfaces as "Task 'X' not found" from taskService.transition, but the worktreeService.remove would be called first (potentially failing on a non-existent worktree).
