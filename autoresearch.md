...
- Latest direction in progress: reduce the remaining manual friction in the instant→plan fallback by returning draft reviewed-plan scaffolds from `warcraft_task_create` / `warcraft_status`, especially once work has already been identified as lightweight/standard or has outgrown the tiny-task path.
