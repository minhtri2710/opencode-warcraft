...
- Extended the same structured-failure pattern to `warcraft_task_expand` when a requested lightweight promotion violates guardrails, returning validation issues plus machine-readable retry args for the standard reviewed path.
- Extended structured failure metadata to `warcraft_plan_write` discovery-gate failures too, so invalid draft plan content now returns machine-readable blocker details (`blockedReason`, `discoveryError`, retry metadata, warnings) instead of only prose.
- Extended structured failure metadata to `warcraft_task_expand` merge failures against malformed draft plans, so missing `## Tasks` sections now return machine-readable repair and retry args instead of only a prose error.
- Extended structured failure metadata to `warcraft_task_expand` invalid-selection failures too, so requesting a missing manual task now returns available pending manual tasks plus machine-readable retry args instead of only a prose error.
- Extended structured continuation metadata to `warcraft_plan_write({ useScaffold: true })` and `warcraft_task_expand` when there is nothing left to promote into a draft, so those calls now hand back the next review/approve/sync flow instead of stopping at a dead-end error.
- Extended structured recovery metadata to `warcraft_task_expand` when expansion is attempted against a non-draft plan, so the tool now returns machine-readable revision and retry args instead of only saying to revise the plan manually.
- Extended the same structured recovery pattern to `warcraft_tasks_sync` when a plan is still draft, so an early sync attempt now returns machine-readable approval/sync handoff instead of a bare 'approve first' error.
- Extended `warcraft_tasks_sync` again for the no-plan case when pending manual work exists, so an early sync attempt on instant/manual work now returns machine-readable promotion/approve/sync handoff instead of only saying there is no `plan.md`.
...
- Extended `warcraft_task_expand` when an existing draft plan has a `## Tasks` section but still lacks required discovery details, so merge attempts now return machine-readable draft-repair metadata instead of only bubbling the discovery error prose.
- Extended `warcraft_task_expand` for the approved-plan/no-pending-work case, so calling expand after promotion is already complete now returns the exact `warcraft_tasks_sync` continuation handoff instead of incorrectly steering users back toward plan revision.
